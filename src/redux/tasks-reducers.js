...
export const updateNewTaskText = (text) => ({ type: UPDATE_NEW_TASK_TEXT, text })
...
export const updateNewTaskBody = (taskId, text) => {
    return (dispatch) => {
        tasksApi.updateNewTaskBody(taskId, text)
            .then(response => {
                if (response) {
                    dispatch(updateNewTaskBodyAction(taskId, text))
                }
            })
    }
};
